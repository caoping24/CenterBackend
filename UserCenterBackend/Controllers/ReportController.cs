...
namespace CenterBackend.Controllers
{
    ...
    public class ReportController : ControllerBase
    {
        ...
        [HttpPost]
        public async Task<BaseResponse<bool>> AddReport([FromBody] AddReportDto_1 AddReportDto)
        {
            if (AddReportDto.Target.IsNullOrEmpty())
            {
                throw new BusinessException(ErrorCode.PARAMS_ERROR, "用户账号不能为空");
            }
            var result = await reportService.AddReport(AddReportDto);
